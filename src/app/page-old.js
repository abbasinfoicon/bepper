...
export default function Home() {
    ...
    const circleRefs = useRef(Array.from({ length: 10 }, () => createRef()));

    const generateCircles = (src, count) => (
        Array.from({ length: count }, (_, index) => (
            <Draggable key={index} handle=".dragImgHandle" nodeRef={circleRefs.current[index]}>
                <div className={`circle circle${index}`} ref={circleRefs.current[index]}>
                    <div className="dragImgHandle">Drag</div>
    ...
    useEffect(() => {
        const circles = generateCircles(circleImageSources[color], 10);
                                    ...
                                    <div className="img-full" style={{ transform: `scale(${scale / 100 * 2})` }}>
                                        ...
                                        <div className={`Bord_GD ${view.witBoard ? 'hide' : 'show'}`}>
                                            ...
                                            <div className={`bepper-bx cntr skyblue ${view.meTime || showGem ? 'show' : 'hide'}`}>
                                                <div className="first">{circleElements}</div>
                                                <div className="second">{circleElements}</div>
                                            </div>
                                        </div>
